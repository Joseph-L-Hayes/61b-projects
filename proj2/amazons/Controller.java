...
final class Controller {
    ...
    private Command[] _commands = {
        ...
        new Command("move$", this::doMove),
        new Command(
                "([a-j])([1-9]|10)\\s([a-j])([1-9]|10)\\s([a-j])([1-9]|10)$",
                this::doMove),
        new Command(
                "([a-j])([1-9]|10)(\\-)([a-j])"
                        + "([1-9]|10)(\\()(([a-j])([1-9]|10)(\\)))$",
                this::doMove),
        new Command("auto\\s+(black|white)$", this::doAuto),
        new Command("manual\\s+(black|white)$", this::doManual)

    };


    /** Command "auto".
     * @param player the player to change to AI. */
    private void doAuto(Matcher player) {

        if (player.group(1).equals("white")) {
    ...
    /** Command "manual".
     * @param player The player to change to manual. */
    ...
    /** Command "from-to-spear.
     * @param move the move to perform. */
    private void doMove(Matcher move) {

        Move currentMove = Move.mv(move.group());
