...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        Iterator<Integer> seg = segment.iterator();
        final String format1 = "%d. Take %s %s for %.1f miles. %n";
        final String formatFinal = "%d. Take %s %s for %.1f miles to %s. %n";

        int first = seg.next();
        int next = seg.next();
        Road roadOne = _map.getLabel(first, next);
        Direction dirOne = roadOne.direction();
        Direction dirTwo = null;
        double distanceOne = roadOne.length();
        Location dest = null;
        String currDir = "";
        ...
        while (seg.hasNext()) {
            ...
            Road roadTwo = _map.getLabel(first, next);
            dirTwo = roadTwo.direction();
            currDir = roadOne.direction().fullName();
            double distanceTwo = roadTwo.length();

            if (roadOne.toString().equals(roadTwo.toString())
                    && dirOne.equals(dirTwo)) {
                distanceOne += distanceTwo;
            } else {
                System.out.printf(format1, seq, roadOne, currDir, distanceOne);
                seq++;
                roadOne = roadTwo;
                dirOne = dirTwo;
                distanceOne = distanceTwo;
            }
            dest = _map.getLabel(next);
        }
        System.out.printf(formatFinal, seq, roadOne,
                currDir, distanceOne, dest.toString());
        seq++;
        return seq;
    }
    ...
    private void addRoad(String from, String name, double length,
        ...
        Road roadFrom = new Road(name, dir.reverse(), length);
        _map.add(v0, v1, roadTo);
        _map.add(v1, v0, roadFrom);
    }
    ...
    private class TripPlan extends SimpleShortestPaths {
        ...
        /** Returns an estimated heuristic weight of the shortest path from
         * vertex V to the destination vertex (if any).  This is assumed to
         * be less than the actual weight, and is 0 by default. */
        @Override
